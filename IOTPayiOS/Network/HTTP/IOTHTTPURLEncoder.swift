...
public struct IOTURLEncoder {

	static func encodeParameters(for urlRequest: inout URLRequest, with parameters: HTTPParameters) throws {
		if parameters == nil { return }
		guard let url = urlRequest.url, let unwrappedParameters = parameters else { throw IOTHTTPNetworkError.missingURL }
	...
	static func setHeaders(for urlRequest: inout URLRequest, with headers: HTTPHeaders) throws {
		...
		guard let unwrappedHeaders = headers else { throw IOTHTTPNetworkError.headersNil }
